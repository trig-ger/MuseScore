...
int makeDynamicProgrammingStep(
      ...
      for (int i = 0; i != (int)penalties.size(); ++i) {
            if (i != (int)penalties.size() - 1)    // if tuplet
            ...
            if (pos == 0) {
                  ...
                  for (int j = 0; j != (int)penalties.size(); ++j) {
                        if ((j == i && j != (int)penalties.size() - 1) || !penalties[j][pos - 1].isValid)
                              continue;
...
std::vector<int> collectOptimalTuplets(
            const std::vector<std::vector<PenaltyParams>> &penalties,
            const std::vector<std::vector<int>> &optimalPaths,
            int finalIndex)
      {
      std::vector<int> bestIndexes;

      const size_t count = penalties[0].size();
      int index = finalIndex;

      for (size_t i = count - 1; ; --i) {
            if (index < (int)penalties.size() - 1)   // if it is a tuplet index
...
void filterTuplets(std::vector<TupletInfo> &tuplets)
      ...
      if (false && permutationSolutionSize <= dynamicSolutionSize) {
            ...
            while (true) {
                  ...
                  // find optimal tuplet combination via dynamic programming
                  // rows = tuplets.size + 1 because there is also a possibility of no-tuplet choice
            std::vector<std::vector<PenaltyParams>> penalties(tuplets.size() + 1);
            for (size_t i = 0; i != tuplets.size() + 1; ++i)
                  penalties[i].resize(tuplets.size());
            std::vector<std::vector<int>> optimalPaths(tuplets.size() + 1);      // first col is unused
            for (size_t i = 0; i != tuplets.size() + 1; ++i)
                  optimalPaths[i].resize(tuplets.size());

            int finalIndex = 0;
            for (int pos = 0; pos != (int)tuplets.size(); ++pos) {
                  finalIndex = makeDynamicProgrammingStep(penalties, optimalPaths, pos,
                                                          tuplets, tupletIntervals);
                  }
                        // get the optimal tuplets found via dynamic programming
            bestIndexes = collectOptimalTuplets(penalties, optimalPaths, finalIndex);
            }
