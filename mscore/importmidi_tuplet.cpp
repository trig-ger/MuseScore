...
struct TupletCommon
      ...
      std::vector<int> *levelIndexes = nullptr;   // selected indexes for some level of the recursion
      };
...
bool isInCommonIndexes(
            ...
            size_t endShift)
      {
      for (size_t i = 0; i != selectedCommons.size() - endShift; ++i) {
...
bool isFirstChordOccupied(
      ...
      for (size_t i = 0; i != selectedCommons.size(); ++i) {
...
bool isCompatibleWithCurrent(
            int indexToCheck,
            const std::vector<TupletCommon> &tupletCommons,
            const std::vector<int> &currentLevelIndexes)
      {
      for (size_t i = 1; i != currentLevelIndexes.size(); ++i) {
            const auto &nextIndexes = tupletCommons[currentLevelIndexes[i]].commonIndexes;
...
void addIndexToUpperLevels(int indexToAdd,
                           const std::vector<int> &selectedCommons,
                           const std::vector<TupletCommon> &tupletCommons)
      {
      for (size_t i = 0; i != selectedCommons.size() - 1; ++i) {
            auto &commmon = tupletCommons[selectedCommons[i]];
            auto &indexes = commmon.commonIndexes;
            if (indexes.find(indexToAdd) == indexes.end())
                  commmon.levelIndexes->push_back(indexToAdd);
            }
      }
...
void findNextTuplet(
      ...
      if (selectedCommons.empty())
            currentLevelIndexes.push_back(0);
      else {
            for (int indexToCheck: tupletCommons[selectedCommons.back()].uncommonIndexes) {
                        // endShift == 1 because the indexToCheck cannot have common indexes
                        // with selectedCommons.back() due to indexToCheck is
                        // in uncommonIndexes of selectedCommons.back()
                  if (isInCommonIndexes(indexToCheck, selectedCommons, tupletCommons, 1)
                              || isFirstChordOccupied(indexToCheck, selectedCommons,
                                                      tupletCommons, tuplets)
                              || areAllVoicesOccupied(indexToCheck, selectedCommons,
                                                      tupletCommons, tupletIntervals, tuplets))
                        {
      ...
      const auto &firstIndexes = tupletCommons[currentLevelIndexes[0]].commonIndexes;
      for (int indexToCheck: firstIndexes) {
            if (isCompatibleWithCurrent(indexToCheck, tupletCommons, currentLevelIndexes))
                  continue;
            if (isInCommonIndexes(indexToCheck, selectedCommons, tupletCommons, 0)
                        || isFirstChordOccupied(indexToCheck, selectedCommons,
                                                tupletCommons, tuplets)) {
                  continue;
                  }
